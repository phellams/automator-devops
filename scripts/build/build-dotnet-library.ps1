...
$interLogger.invoke($logname, "Running Build on DotNet {kv:Library=$Name} {kv:Version=$package_version}", $false, 'info')
...
$interlogger.invoke($logname, "Successfully built DotNet ", $false, 'info')
...
$interLogger.invoke($logname, "Packing DotNet library into NuGet package...", $false, 'info')
...
$interLogger.invoke($logname, "Successfully packed DotNet library into NuGet package", $false, 'info')  

#debug - Copy nuget package to root \dist\dotnet for packaging as build artifact and deployment
$interLogger.invoke($logname, "Copying NuGet package to ../dist/dotnet for build artifact and deployment...", $false, 'info')
