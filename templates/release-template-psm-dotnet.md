...
### **░░░░▌ DOTNET SHA256 Checksums ▐░░░░**

REPONAME_PLACE_HOLDER.VERSION_AND_PRERELEASE_PLACE_HOLDER.nupkg SHA256: `⦿` **`NUGET_NUPKG_HASH`** \

## `⦿` DOTNET Library Packages

Libraries are distributed to **Nuget.org** repository.

### **Nuget.org**

🔸**via `dotnet`**
```dotnet
dotnet add package REPONAME_PLACE_HOLDER --version=VERSION_AND_PRERELEASE_PLACE_HOLDER
```

🔸**via `nuget`**
```powershell
# add nuget source
nuget sources add -name GITGROUP_PLACE_HOLDER_REPONAME_PLACE_HOLDER -source https://nuget.org/api/v2/package/REPONAME_PLACE_HOLDER

# A: Install from nuget package into current directory
nuget install REPONAME_PLACE_HOLDER PRERELEASE_NUGET_PLACE_HOLDER -version VERSION_AND_PRERELEASE_PLACE_HOLDER -Source GITGROUP_PLACE_HOLDER_REPONAME_PLACE_HOLDER

# B: Install from nuget package into user profile
nuget install REPONAME_PLACE_HOLDER PRERELEASE_NUGET_PLACE_HOLDER -Source nuget.org -OutputDirectory $ENV:USERPROFILE/documents/powershell
```


## `⦿` PSMODULE Packages 

Modules are distributed to **GitLab Packages**, **Chocolatey Packages**, and **Powershell Gallery** repositories.
...
For all module output variations, you can simply extract the `.zip` files, or rename `.nupkg` files to `.zip`, then extract them using your preferred compression tool (e.g., **ZIP**, **PeaZip**, **7-Zip**, etc.). After extracting:

- if **PSModule** navigate to the module directory (`cd`) and run Import-Module. Alternatively, you can use any of the methods mentioned above or below.
- if **dotnet** navigate to the specific library directory (`cd`) and and find the dlls in the `bin/Release` folder.

Or you can use the individual build artifacts to install the module or source dlls, using the target package manager ie:  
